...
const SecurityLogs = () => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/admin/logs/security?${queryParams}`, {
    ...
    <div className="products-manager">
      <div className="manager-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, color: '#666', fontSize: '14px' }}>Monitor security events, failed logins, and suspicious activities</p>
          </div>
        </div>
      </div>
      ...
      <div className="manager-filters" style={{ display: 'flex', gap: '16px', marginBottom: '24px', flexWrap: 'wrap' }}>
        <div className="search-bar" style={{ flex: 1, minWidth: '250px', position: 'relative' }}>
          ...
          <input 
            type="text" 
            placeholder="Search event type, IP, identifier, details..." 
            value={search}
          ...
          <div className="table-responsive">
            <table className="admin-table">
              ...
              <tbody>
                {logs.length === 0 ? (
                  <tr>
                    <td colSpan="5" style={{ textAlign: 'center', padding: '24px' }}>No security logs found</td>
                  </tr>
                ) : (
                  logs.map(log => (
                    <tr key={log.id}>
                      ...
                      <td>
                        <span style={{ 
                          ...
                          fontSize: '12px', 
                          fontWeight: 'bold',
                          backgroundColor: log.event_type.toLowerCase().includes('fail') ? '#fee2e2' : '#e0e7ff',
                          color: log.event_type.toLowerCase().includes('fail') ? '#b91c1c' : '#4338ca'
                        }}>
                          {log.event_type}
                        </span>
                      </td>
